...
export function UserTableSkeleton({ showFilters = false }: UserTableSkeletonProps) {
  return (
    <div className="w-full space-y-4">
      {/* Only show filter skeleton if explicitly requested (for initial load) */}
      {showFilters && (
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Skeleton className="h-10 w-64" /> {/* Search input */}
            <Skeleton className="h-10 w-24" />  {/* Role filter */}
